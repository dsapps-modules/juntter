<footer class="footer-juntter">
    <div class="container">
        <div class="row">
            <div class="col-lg-4 col-md-6 mb-4">
                <div class="footer-section">
                    <h5>Juntter</h5>
                    <p>Checkout digital que vende por você. Simplifique seus pagamentos online com segurança e praticidade.</p>
                    <div class="mt-3">
                        <a href="#" class="mr-3"><i class="fab fa-instagram"></i></a>
                        <a href="#" class="mr-3"><i class="fab fa-linkedin"></i></a>
                        <a href="#"><i class="fab fa-twitter"></i></a>
                    </div>
                </div>
            </div>
            <div class="col-lg-2 col-md-6 mb-4">
                <div class="footer-section">
                    <h5>Produto</h5>
                    <a href="{{ route('checkout') }}#beneficios">Benefícios</a><br>
                    <a href="{{ route('checkout') }}#precos">Preços</a><br>
                    <a href="{{ route('checkout') }}#como-funciona">Como Funciona</a><br>
                    <a href="#">API</a>
                </div>
            </div>
            <div class="col-lg-2 col-md-6 mb-4">
                <div class="footer-section">
                    <h5>Empresa</h5>
                    <a href="#">Sobre Nós</a><br>
                    <a href="#">Carreiras</a><br>
                    <a href="#">Imprensa</a><br>
                    <a href="#">Blog</a>
                </div>
            </div>
            <div class="col-lg-2 col-md-6 mb-4">
                <div class="footer-section">
                    <h5>Suporte</h5>
                    <a href="{{ route('checkout') }}#faq">FAQ</a><br>
                    <a href="#">Central de Ajuda</a><br>
                    <a href="#">Contato</a><br>
                    <a href="#">Status</a>
                </div>
            </div>
            <div class="col-lg-2 col-md-6 mb-4">
                <div class="footer-section">
                    <h5>Legal</h5>
                    <a href="#">Privacidade</a><br>
                    <a href="#">Termos</a><br>
                    <a href="#">Segurança</a><br>
                    <a href="#">Compliance</a>
                </div>
            </div>
        </div>
        <hr style="border-color: #444; margin: 40px 0 20px;">
        <div class="row align-items-center">
            <div class="col-md-6">
                <p class="mb-0">&copy; {{ date('Y') }} Juntter. Todos os direitos reservados.</p>
            </div>
            <div class="col-md-6 text-md-right">
                <p class="mb-0"> (11) 4003-5442 |  info@juntter.com.br</p>
            </div>
        </div>
    </div>
</footer>

<div class="chat-widget" onclick="openChat()">
    <i class="fas fa-comments" style="color: var(--dark-color); font-size: 1.5rem;"></i>
</div> 